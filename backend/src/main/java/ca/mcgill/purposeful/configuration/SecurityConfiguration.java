...
public class SecurityConfiguration {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
            .csrf().disable() // csrf protection is an extra security layer - prevent cross site request forgery. Adds extra complexity. For post requests, you need extra actions
            .oauth2ResourceServer().jwt().jwtAuthenticationConverter(new AuthenticationConverter()).and().and() // configuring your project to accept jwt tokens as a method of authentication. Jwt (Json Web Token) tokens are json objects as strings, with no spaces, base64 encoded: headers, payhold, signature
            .authorizeHttpRequests().anyRequest().authenticated().and() // by default, all endpoints are authenticated
            ...
            .httpBasic().and() // another way - in hhtp request that the backend sends, in headers there is "Authorization":"Basic encodeBase64(username:password)" ( encodebase64 is a method in javascript method that does base 64 encoding )
    ...
    /**
     * Takes in public key, and uses it to decode the JWT. Decoded from base64 to ascii so that it can be validated
