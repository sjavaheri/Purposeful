Feature: Modify Idea
  As the user, I want to make changes to my idea so that my idea can have updated information for people viewing it.
  ...
  Scenario Outline: Successfully modify an idea
	...
	Scenario Outline: (Alternate Flow) Successfully modify the idea with an empty field that can be empty 
	...
	Scenario Outline: (Error Flow) Unsuccessfully modify the idea with an empty field that cannot be empty
		When the user requests to modify the field <field> to become empty for idea with id <idea_id>
	...
	Scenario Outline: (Error Flow) Unsuccessfully modify an idea with a non-existing object
		When the user requests to modify the field <field> to become <new value> for idea with id <idea_id>
		Then the idea with id <idea_id> will have value <old_value> for the field <field>
		Then the error message <error> will be thrown with status code <Http_status>
